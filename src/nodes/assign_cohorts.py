...
def assign_cohorts(state: dict) -> dict:
    ...
    try:
        ...
        cur.execute(
            ...
            "AND NOT EXISTS (SELECT 1 FROM channel_cohorts cc WHERE cc.channel_id = c.channel_id "
            "AND cc.vertical = 'crime') "
            "LIMIT 200"
        )
        channels = cur.fetchall()
        cur.close()

        for ch_id, subs, ch_creation, v_start, eng, eg, vertical in channels:
            ...
            is_under = eng < 20 and subs < 100000

            if vertical == "crime":
                cohort_code = "underperformer" if (is_under and not is_new) else (
                ...
                cohort_code = "new_winner" if (is_new and is_winner) else (
                    "new_loser" if is_new else ("new_winner" if is_winner else None)
                )
            else:
                continue
